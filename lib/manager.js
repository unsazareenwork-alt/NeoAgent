...
async function cmdLoginClaudeCode() {
  ...
  const authCode = await new Promise((resolve, reject) => {
    ...
    server.listen(80, 'localhost', () => {
      logInfo('Waiting for OAuth callback on http://127.0.0.1/callback ...');
    });
    server.on('error', (err) => {
      clearTimeout(timeout);
      const hint = err.code === 'EACCES'
        ? 'Port 80 requires elevated privileges. Try: sudo neoagent login claude-code'
        : err.message;
      reject(new Error(`Could not start OAuth callback server: ${hint}`));
    });
  });
