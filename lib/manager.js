...
async function ensureQemuInstalled() {
  ...
  if (platform === 'macos') {
    ...
    if (commandExists('apt-get')) {
      logInfo('Running "sudo apt-get update && sudo apt-get install -y qemu-system-x86 qemu-utils"...');
      runOrThrow('sudo', ['apt-get', 'update']);
      runOrThrow('sudo', ['apt-get', 'install', '-y', 'qemu-system-x86', 'qemu-utils']);
      ...
      throw new Error('Unsupported Linux distribution. Please install qemu-system-x86 and qemu-utils manually.');
    }
  } else {
